...
def get_output_dir(datasets, training=True):
    ...
    assert isinstance(datasets, (tuple, list, six.string_types)), \
...
def load_cfg(cfg_to_load):
    ...
    assert isinstance(cfg_to_load, (io.IOBase, six.string_types)), \
        'Expected {} or {} got {}'.format(file, str, type(cfg_to_load))
    if isinstance(cfg_to_load, io.IOBase):
        # cfg_to_load = ''.join([ x.decode() for x in cfg_to_load.readlines()])
        cfg_to_load = ''.join(cfg_to_load.readlines())
    if isinstance(cfg_to_load, six.string_types):
